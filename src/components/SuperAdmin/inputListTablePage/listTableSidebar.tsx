...
const TablePageSideBar = (props: any) => {
    ...
    const goToAnalytics = () => {
        ...
        navigate("/super-admin/Table-List/" + localStorage.getItem('moduleId') + "/dashboards")
    }

    return (
        <div className="" style={{ background: "gainsboro", height: "100%" }}>
            <section>
                {
                    (localStorage.getItem('moduleName') !== 'Analytics' &&
                        ...
                        <div className='m-3 mt-0 pt-4'>
                            <div>Dashboards <i className="cursor-pointer pi pi-plus-circle ml-4" onClick={goToAnalytics}></i> </div>
                        </div>
                    )
                }
            </section>
